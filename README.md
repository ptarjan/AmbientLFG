...
- **Rules**: "alert me for Mythic Lura groups that still need a tank" is `/pma add mythic lura +tank` (or build it in the UI). Words match the boss/activity and are spelling-tolerant ("Lurra" matches "lura").
...
- **Live matches list**: the `/pma` window shows every currently-listed matching group with its tank/healer/dps counts, boss, difficulty, and title.
- **Seller filtering**: boost/carry advertisers are recognized and hidden automatically; block any leader forever with one click.

## Usage

1. `/pma` to open the settings window
